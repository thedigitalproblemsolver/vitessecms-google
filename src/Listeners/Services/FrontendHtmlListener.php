    ...
    public function parseHeader(Event $event)
    {
        if(!$this->isAdminPage && !$this->hasAdminAccess && $this->environment === EnvEnum::ENVIRONMENT_PRODUCTION) {
            if ($this->settingService->has(GoogleEnum::GOOGLE_TAGMANAGER_ID)) {
                $this->assetsService->addHeadCode($this->eventsManager->fire(ViewEnum::RENDER_TEMPLATE_EVENT,
                    new RenderTemplateDTO(
                        'tag_manager_head',
                        '',
                        [GoogleEnum::GOOGLE_TAGMANAGER_ID => $this->settingService->getString(GoogleEnum::GOOGLE_TAGMANAGER_ID)]
                    )
                ));
            }
            if ($this->settingService->has(GoogleEnum::GOOGLE_ADSENSE_AUTOMATICADS)) {
                $this->assetsService->addHeadCode($this->eventsManager->fire(ViewEnum::RENDER_TEMPLATE_EVENT,
                    new RenderTemplateDTO(
                        ...
                        '',
                        [GoogleEnum::GOOGLE_ADSENSE_AUTOMATICADS => $this->settingService->getString(GoogleEnum::GOOGLE_ADSENSE_AUTOMATICADS)]
                    )
                ));
            }
            if ($this->settingService->has(GoogleEnum::GOOGLE_SITE_VERIFICATION)) {
                $this->assetsService->addHeadCode($this->eventsManager->fire(ViewEnum::RENDER_TEMPLATE_EVENT,
                    new RenderTemplateDTO(
                        ...
                        '',
                        [GoogleEnum::GOOGLE_SITE_VERIFICATION => $this->settingService->getString(GoogleEnum::GOOGLE_SITE_VERIFICATION)]
                    )
                ));
            }
        }
